...
public interface ProductRepository  extends CrudRepository<Product, Integer>{
    ...
    public int updateQuantity(@Param("productID") int productId, @Param("quantity") int quantity);
      ...
      // will trigger the query annotated to the method as updating query instead of a selecting one
      @Modifying
      @Query("Update Product p SET p.totalq = :totalq WHERE p.productID = :productID")
      public int updateTotalq(@Param("productID") int productId, @Param("totalq") int totalq);
      
     /* @Transactional
      // will trigger the query annotated to the method as updating query instead of a selecting ones
      @Modifying
      @Query(" SELECT p FROM Product p WHERE p.quantity > :trashq")
